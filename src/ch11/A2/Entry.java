...
public abstract class Entry {
    ...
    public String getFullName() {
        ...
        do {
            fullname.insert(0, entry.getName());//현재 객체의 이름을 맨앞에 삽입함
            fullname.insert(0, "/");//entry변수는 현재 노드를 가리킴
            entry = entry.parent;//이게 젤 중요. 현재 위치(포인터)를 부모로 옮긴다.
        } while (entry != null);//부모가 더이상 없으면 루프 빠져나옴
